...
graphics() {
	...
	case "$DE" in
		"Simpl-Arch-Xfce") 	DE="xfce4 xfce4-goodies gvfs zsh zsh-syntax-highlighting"
...
config_env() {
	...
	cp -r "/usr/share/arch-anywhere/desktop/Numix" "$ARCH"/usr/share/themes/
			...
			case "$software_menu" in
				"$aar")
					software=$(dialog --ok-button "$ok" --cancel-button "$cancel" --checklist "$software_msg1" 14 60 4 \
						"arch-wiki-docs"		"$aar0" ON 3>&1 1>&2 2>&3)
					if [ "$?" -gt "0" ]; then
						err=true
					fi
				;;
				"$audio")
					software=$(dialog --ok-button "$ok" --cancel-button "$cancel" --checklist "$software_msg1" 20 60 10 \
						...
						"ncmpcpp"		"$audio7" OFF \
						"pianobar"		"$audio8" OFF \
						"pulseaudio"	"$audio9" OFF 3>&1 1>&2 2>&3)
					if [ "$?" -gt "0" ]; then
						err=true
					fi
				;;
				...
				;;
				"$webapps")
					software=$(dialog --ok-button "$ok" --cancel-button "$cancel" --checklist "$software_msg1" 21 65 10 \
						"apache"		    "$web1" ON \
						"apache-ant"		"$web2" OFF \
						"cordova"		    "$web3" OFF \
						"mariadb"			"$web4" ON \
						"mysql-workbench"	"$web5" OFF \
						"php"			    "$web6" ON \
						"php-apcu"			"$web7" OFF \
						"php-apcu-bc"		"$web8" OFF \
						"php-cgi"			"$web9" OFF \
						"php-fpm"			"$web10" OFF \
						"php-gd"			"$web11" OFF \
						"phpmyadmin"		"$web12" ON \
						"xdebug"			"$web13" OFF \
						"composer"		    "$web14" OFF \
						"jre8-openjdk"		"$web17" OFF \
						"jdk8-openjdk"		"$web18" OFF \
						"nodejs"			"$web19" OFF \
						"git"			    "$web20" ON \
						"gitlab"			"$web21" OFF 3>&1 1>&2 2>&3)
					if [ "$?" -gt "0" ]; then
						err=true
					fi
				;;
				"$webtools")
					software=$(dialog --ok-button "$ok" --cancel-button "$cancel" --checklist "$software_msg1" 12 65 10 \
						"bluegriffon"		"$too1" OFF \
						"kompozer"			"$too2" OFF \
						"anjuta"			"$too3" OFF \
						"arduino"			"$too4" OFF \
						"intellij-idea-community-edition"			"$too5" OFF \
						"retext"			"$too6" OFF \
						"monodevelop"		"$too7" OFF \
						"bluefish"		    "$too8" OFF \
						"codeblocks"		"$too9" ON \
						"netbeans"			"$too10" ON \
						"eclipse-common"			"$too11" ON \
						"eclipse-php"		"$too12" OFF 3>&1 1>&2 2>&3)
					if [ "$?" -gt "0" ]; then
						err=true
					fi
				;;
